            ...
            for (ComSignalIndex=0; Com.ComConfig.ComIPdu[ComIPduIndex].ComIPduSignalRef[ComSignalIndex] != NULL; ComSignalIndex++)
            {
                if (Com.ComConfig.ComIPdu[ComIPduIndex].ComIPduSignalRef[ComSignalIndex]->ComHandleId == SignalId)
                {
                    ...
                    if(ComCopySignal)
                    {
                        ...
                        send_packet.append((*(unsigned char*)ComSignalLocal->ComBufferRef ));
                      
                        //send_packet.insert(0, *(unsigned char*)ComSignalLocal->ComBufferRef);
                        ...
                        send_packet = send_packet + QByteArray::number(ComIPduLocal->ComIPduHandleId)   + QByteArray::number(ComSignalLocal->ComHandleId);
